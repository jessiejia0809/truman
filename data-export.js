  ...
  for (const user of users) {
    ...
    for (const userPost of user.posts) {
      let string =
        userPost.body +
        ", on Day " +
        (Math.floor(userPost.relativeTime / 86400000) + 1) +
        "\r\n";
    ...
    for (const feedAction of user.feedAction) {
      ...
      const NewComments = feedAction.comments.filter(
      ...
      for (const newComment of NewComments) {
        let string =
          newComment.body +
          ", on Post " +
          feedAction.post.postID +
          ", on Day " +
          (Math.floor(newComment.relativeTime / 86400000) + 1) +
          "\r\n";
        UserCommentsCreated += string;
      }

      const CommentsLiked_list = feedAction.comments.filter(
        (comment) => !comment.new_comment && comment.liked,
      );
      NumActorCommentsLiked += CommentsLiked_list.length;
      if (NumActorCommentsLiked > 0) {
        for (const likedComment of CommentsLiked_list) {
          ActorCommentsLiked.push(
            feedAction.post.comments.find((comment) =>
              likedComment.comment.equals(comment._id),
            ).commentID,
          );
        }
      }

      const CommentsFlagged_list = feedAction.comments.filter(
        (comment) => !comment.new_comment && comment.flagged,
      );
      ...
      if (NumActorCommentsFlagged > 0) {
        for (const flaggedComment of CommentsFlagged_list) {
          ActorCommentsFlagged.push(
            feedAction.post.comments.find((comment) =>
              flaggedComment.comment.equals(comment._id),
            ).commentID,
          );
        }
      }
    ...
    for (const reportedActor of user.reported) {
      const reportLogs = user.blockReportAndFollowLog.filter(
        (log) => log.action == "report" && log.actorName == reportedActor,
      );
      for (const reportReason of reportLogs) {
        actorsReported +=
          reportedActor +
          ", Reported for " +
          reportReason.report_issue +
          "\r\n";
      }
    }
