...
async function openActorChat(username) {
    ...
    const data = await $.getJSON("/chat", { "chatFullId": getChatFullId(username, chat.userId), "chatId": username });

    // Update chat header with given actor metadata
    $(".actor-chat").attr("id", data["username"]);
    $(".actor-chat .chat .chat-header img.ui.avatar.image").attr("src", data["picture"]);
    $(".actor-chat .chat .chat-header .chat-about .chat-with").text("Chat with " + (!data["name"] ? `@${data["username"]}` : data["name"]));
...
function clickMessageUser(event) {
    ...
    const username = target.parent().siblings(".header").attr("data-username").trim();
...
$(window).on("load", function() {
                ...
                if (msg.chatId == $('.actor-chat').attr("userId")) {
                    ...
                    if (chat) {
                        ...
                        if (chat.chatId != msg.senderUsername) {
                            await openActorChat(msg.senderUsername, msg.senderName, msg.senderSrc);
                        } else {
            ...
            socket.on("chat typing", async function(msg) {
                ...
                if (msg.chatId == $('.actor-chat').attr("userId")) {
                    ...
                    if (chat) {
                        ...
                        if (msg.senderUsername == chat.chatId && $('.actor-chat .chat .chat-history').is(":visible")) {
            ...
            $('.username').popup({
                ...
                onShow: function(el) {
                    ...
                    const picture = $(el).parent().siblings(".avatar").find("img").attr("src") || $(el).parents(".content").siblings(".avatar").find("img").attr("src");
                    const isSelf = $(el).attr("data-self") == "true";
                    if (!isSelf) {
                        $(this).html(`<div class='header' data-username=${username}><img class='ui avatar image' src='${picture}'/>${name}</div>
                        <div class='content'>
            ...
            $('.container.clearfix').each(function() {
                        ...
                        const chat = {
                                ...
                                // Renders a message
                                render: function(body, absTime, username, isExternalMessage, isTypingAnimation) {
