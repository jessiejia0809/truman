...
const scriptSchema = new mongoose.Schema({
    postType: { type: String, required: true, enum: ['Actor', 'User'] }, //Indicates whether an actor or user made a post
    poster: { type: Schema.ObjectId, refPath: 'postType' }, // Indicates which actor/user made the post
    postID: Number, // ID for post (0,1,2,3...)
    class: { type: String, default: '', trim: true }, // For experimental use (If blank/null, this post is shown to all users. If defined, this post is only shown to users with the same value for their experimental condition)
    body: { type: String, default: '', trim: true }, // Text(body) of post
    picture: String, // Picture (file path) for post
    likes: { type: Number, default: 0 }, // Indicates the number of likes on the post 
